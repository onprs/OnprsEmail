...
- **基础运行环境**：
  - 容器引擎：Docker & Docker Compose (v5.x)
...
2. **入站邮件架构（Inbound Ingress）**：
   - 采用 **Cloudflare Email Routing (Catch-all 通配规则) + Email Worker + Server Ingress Gateway** 架构；
   - 发往 `*@onprs.online` 的所有邮件由 Cloudflare 全球邮件网络接收，触发 Worker 通过 HTTPS POST 直投至服务端 `email-ingress` 网关；
   - 网关校验 `X-Ingress-Secret` 后通过内部 SMTP 直接入库 Stalwart 与 PostgreSQL；
   - 实现**全域通配、免 25 端口依赖、秒级全自动入库**。
...
1. **Stalwart Mail Server (`stalwart-mail`)**：
   - 现代化 Rust 全合一邮件核心，挂载 PostgreSQL 18 存储全部邮件元数据与用户体系；
   - 接入外部 Docker 网络 `1panel-network` 与内部 `mail-network`。
2. **Ingress 接收网关 (`email-ingress-gateway`)**：
   - 极轻量 Python 标准库微服务，监听 `127.0.0.1:4082`；
   - 对接 Cloudflare Worker，负责 Token 鉴权、CRLF 规范化及内部 SMTP 投递。
3. **SnappyMail Webmail (`snappymail-web`)**：
   - 超轻量现代 Webmail 前端，监听 `127.0.0.1:4081`；
   - 预置 `use-mail.onprs.online` / `onprs.online` / `mail.onprs.online` 对接到 `stalwart` 的 993/465 加密通道。
...
| **MX** | `@` | `route1.mx.cloudflare.net` 等 (优先级 21, 25, 33) | - | Cloudflare Email Routing 接收节点 |
| **TXT (SPF)** | `@` | `v=spf1 mx include:spf.sendinblue.com ~all` | - | 授权发信主体（包含中继提供商） |
| **TXT (DKIM)**| `stalwart._domainkey` | `v=DKIM1; k=rsa; p=<Public_Key>` | - | RSA 2048 防篡改签名公钥 |
| **TXT (DKIM)**| `stalwart-ed25519._domainkey` | `v=DKIM1; k=ed25519; p=<Public_Key>` | - | Ed25519 现代签名公钥 |
...
├── services/                # 专有微服务源码
│   └── ingress/             # 通用 Ingress 接收网关
│       ├── Dockerfile       # 极简镜像构建定义
│       └── app.py           # HTTP Ingress 接收与内部 SMTP 投递核心
├── config/                  # 配置文件模版
│   └── snappymail/          # SnappyMail 预置域名与连接配置
│       └── domains/
│           └── onprs.online.json
├── scripts/                 # 运维自动化脚本
│   ├── setup.sh             # 一键部署与环境初始化脚本
...
└── docs/                    # 完整操作指引与架构文档
    ...
    ├── ingress-api.md       # Ingress v2 查询、详情与附件接口
...
3. **安全与敏感数据**：
   - 严禁将任何明文密码、Relay API Key、Ingress Secret、DKIM 私钥提交至 Git 仓库；
   - 统一使用 `.env` 与挂载卷管理敏感参数；
